...
function lib:CheckInspectQueue()
	...
	for name, guid in pairs(inspectQueue) do
		...
		if not unit then
			...
			if UnitIsVisible(unit) and UnitIsConnected(unit) and not UnitCanAttack("player", unit) and not UnitCanAttack(unit, "player") and CheckInteractDistance(unit, 1) and CanInspect(unit) and UnitClass(unit) then
				NotifyInspect(unit)
				break
			else

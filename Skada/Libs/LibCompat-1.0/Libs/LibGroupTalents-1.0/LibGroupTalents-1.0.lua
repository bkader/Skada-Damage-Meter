...
local CanInspect, CheckInteractDistance = _G.CanInspect, _G.CheckInteractDistance
	...
	if not activeTalents or refetch then
		...
		elseif (UnitInRaid(unit) or UnitInParty(unit)) and UnitIsConnected(unit) then
			...
			if not r.talents and not r.requested then
				...
				if not UnitIsVisible(unit) or not (CheckInteractDistance(unit, 1) and CanInspect(unit)) then
					if r.version then
